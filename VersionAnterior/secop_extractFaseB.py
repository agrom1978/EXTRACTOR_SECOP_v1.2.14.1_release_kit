...
# Control de navegador: 1=headless (sin ventana), 0=visible
DEFAULT_HEADLESS = os.environ.get("SECOP_HEADLESS", "0").strip() in {"1","true","True","TRUE","yes","YES"}
...
def fetch_detail_html(constancia: str, headless: bool = DEFAULT_HEADLESS, timeout_ms: int = 120_000) -> str:
    """
...
def extract_record_from_html(html: str, constancia_ok: str = "") -> dict:
    ...
    return {
        "Número de constancia": constancia_ok,
        "Código Registro Presupuestal (CRP)": crp,
        "Registro Presupuestal (RP/CRP) (limpio)": crp,
