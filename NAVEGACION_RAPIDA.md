...
2. [test_cambios.py](test_cambios.py) — Ejecutar tests
3. [CAMBIOS_IMPLEMENTADOS_secop_ui.md](CAMBIOS_IMPLEMENTADOS_secop_ui.md) — 15 min

### Quiero revisar el código
- [constancia_config.py](constancia_config.py) — Módulo centralizado
- [secop_ui.py](secop_ui.py) — Interfaz Flask mejorada
- [secop_ui_backup.py](secop_ui_backup.py) — Código original (para comparar)
...
- Actualizar `secop_extract.py` para usar `constancia_config.py`
...
# Opción 2: Git
git checkout HEAD~1 -- secop_ui.py constancia_config.py
```
...
### Necesito validar
1. Ejecutar: `python test_cambios.py` (8 tests automatizados)
2. Revisar: [test_cambios.py](test_cambios.py) — Casos de cobertura
...
- ZIP generado: ✓ Con XLSXs + reporte errores
...
| Rollback | secop_ui_backup.py | 2 min |
| Validar todo | test_cambios.py | 1 min |
...
**DASHES UNICODE**
→ [constancia_config.py](constancia_config.py) línea 9
...
**TESTS**
→ [test_cambios.py](test_cambios.py) (8 tests)
...
| Algo no funciona | Ejecuta test_cambios.py |
| Necesito desplegar | Consulta GUIA_DESPLIEGUE.md |
| Quiero rollback | Ejecuta: copy secop_ui_backup.py secop_ui.py |
...
**Desarrollador:**
- [ ] Revisar integración con secop_extract.py
