...
## v1.2.14.2-struct - Ajustes anti-bloqueo y mejoras de extraccion
...
## scripts/secop_extract.py v1.2.14.2 - Ajustes de extraccion de "Tipo de Proceso"
...
|---------|------|--------|-------------|
| **scripts/constancia_config.py** | ✨ NUEVO | 115 | Configuración centralizada de constancias SECOP |
| **secop_ui.py** | 🔄 REFACTORIZADO | 506 | Interfaz Flask mejorada con logging y seguridad |
| **scripts/secop_ui_backup.py** | 📌 RESPALDO | 313 | Copia original para rollback |
| **tests/test_cambios.py** | ✨ NUEVO | 155 | Suite de validación automatizada (8 tests) |
...
2. Ejecutar: `python tests/test_cambios.py` (1 min)
...
2. Comparar: Código original vs nuevo usando `scripts/secop_ui_backup.py`
3. Verificar: [tests/test_cambios.py](tests/test_cambios.py) (muestra casos de uso)
...
1. Ejecutar: `copy scripts/secop_ui_backup.py secop_ui.py`
...
```
├─ Imports (12 módulos)
│  ├─ stdlib: os, secrets, logging, time, datetime, pathlib, typing, html
...
Ejecutar: `python tests/test_cambios.py`
...
| **Rollback** | �
 Disponible | scripts/secop_ui_backup.py presente |
...
- [x] Crear scripts/constancia_config.py centralizado
...
- [x] Crear respaldo (scripts/secop_ui_backup.py)
...
2. Ejecutar `tests/test_cambios.py` en CI/CD
3. Desplegar en ambiente de testing

### Mediano Plazo (Próximas 2 Semanas)
1. Actualizar `scripts/secop_extract.py` para usar constancia_config
