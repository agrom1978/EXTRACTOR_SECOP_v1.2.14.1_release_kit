# GUÍA DE DESPLIEGUE: secop_ui.py v1.2.14.2-struct
...
### Archivos Listos
- ✅ `scripts/constancia_config.py` — Nuevo, compilado, integrado
- ✅ `secop_ui.py` — Refactorizado, compilado, validado
- ✅ `scripts/secop_ui_backup.py` — Respaldo disponible
- ✅ `tests/test_cambios.py` — Suite de validación (8 tests)
...
cd "%USERPROFILE%\OneDrive\Escritorio\EXTRACTOR_SECOP_v1.2.14.1_release_kit"
...
COPY scripts/secop_extract.py secop_ui.py scripts/constancia_config.py ./
...
# 3. Construir imagen
docker build -t secop-ui:1.2.14.2-struct .
...
- [ ] Ejecutar `python tests/test_cambios.py` (debe pasar todos)
- [ ] Verificar `scripts/secop_ui_backup.py` existe (rollback)
...
git checkout HEAD~1 -- scripts/constancia_config.py
...
### Opción 3: Manual
1. Eliminar `secop_ui.py` y `scripts/constancia_config.py`
2. Restaurar desde `scripts/secop_ui_backup.py`
...
**Solución:**
```bash
# Verificar que scripts/constancia_config.py está en el directorio correcto
ls scripts/constancia_config.py
...
1. **Integración continua**
   - Ejecutar `tests/test_cambios.py` en cada commit
   - Validar sintaxis con `pylint`

2. **Actualización de `scripts/secop_extract.py`**
   - Importar de `scripts/constancia_config.py`
...
**Soporte:** Revisar archivos de documentación o ejecutar `tests/test_cambios.py` para validar instalación.
