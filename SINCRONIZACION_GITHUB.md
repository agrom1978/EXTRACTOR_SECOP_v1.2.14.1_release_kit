...
1. **secop_extract.py** - Mejoras anti-bloqueo y throttling
...
**Mensaje:**
```
v1.2.15: Mejoras anti-bloqueo, throttling inteligente y documentación

- Refactorización de secop_extract.py: reuso de sesión Playwright
...
2. **Crear tag (opcional):**
   ```bash
   git tag -a v1.2.15 -m "Release 1.2.15: Anti-bloqueo y throttling inteligente"
   git push origin v1.2.15
   ```

3. **Crear Release en GitHub (opcional):**
   - Ir a Releases → Create Release → v1.2.15
